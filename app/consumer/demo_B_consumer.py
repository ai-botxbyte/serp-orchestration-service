...
class DemoBConsumer(BaseAppConsumer):
    """
    Demo B consumer with multi-job routing support.
    
    This consumer routes messages to different jobs based on job_type:
    - job_type='job1' -> DemoB1Job (Analytics & Reporting Workflow)
    - job_type='job2' -> DemoB2Job (Batch Notification Workflow)
    """
    
    def __init__(self):
        """
        Initialize the demo B consumer with multiple job handlers.
        """
    ...
    def _validate_message(self, message_data: dict) -> DemoOrchestrationMessageSchema:
        ...
        try:
            ...
            logger.debug(f"Validated demo B message: {demo_message.data}")
            return demo_message
            
        except (ValueError, TypeError, KeyError) as e:
            # Demo B the error with the raw message data for debugging
